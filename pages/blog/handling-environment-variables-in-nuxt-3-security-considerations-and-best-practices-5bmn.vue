    ...
    <main v-if="article" class="max-w-[1280px] mx-auto px-[20px] mt-[40px] text-black dark:text-white">
        ...
        <div class="mb-12">
            ...
            <div class="flex items-center gap-6 text-sm text-gray-500 dark:text-gray-400">
                ...
                <a :href="`https://dev.to/elvisans/handling-environment-variables-in-nuxt-3-security-considerations-and-best-practices-5bmn`" target="_blank" class="flex items-center gap-1">
        ...
        <div class="flex items-center gap-4 mb-12">
            ...
            <a :href="`https://twitter.com/intent/tweet?text=Handling Environment Variables in Nuxt 3: Security Considerations and Best Practices&url=${currentUrl}`" target="_blank"
        ...
        <div class="flex items-center gap-4 mb-12 prose dark:prose-invert prose-lg">
            <p>You can like and comment this article on <a :href="`https://dev.to/elvisans/handling-environment-variables-in-nuxt-3-security-considerations-and-best-practices-5bmn`"
...
const { setSeo } = useSeo()
setSeo({
    title: `Handling Environment Variables in Nuxt 3: Security Considerations and Best Practices | Ansima's Blog`,
    description: `TL;DR:   Nuxt uses vitejs https://nuxt.com/docs/api/nuxt-config#vite  Any environment variable with a...`,
    image: `https://media2.dev.to/dynamic/image/width=1000,height=500,fit=cover,gravity=auto,format=auto/https%3A%2F%2Fdev-to-uploads.s3.amazonaws.com%2Fuploads%2Farticles%2F0oq5bvrsl2eeduakdva9.jpg`,
    type: 'article'
})
...
onMounted(async () => {
    try {
        const articleResponse = await fetch(`https://dev.to/api/articles/elvisans/handling-environment-variables-in-nuxt-3-security-considerations-and-best-practices-5bmn`)
